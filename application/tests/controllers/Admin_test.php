...
class Admin_test extends TestCase
{
        ...
        // R E A D  D A T A //
        public function test_getDataKomentar_berhasil(){ // GET DATA KOMENTAR
            $_SESSION['username'] = 'admin';
            $output = $this->request('GET', 'My_Controller/readDataKomentar');
            $this->assertContains('<title>KOMENTAR</title>', $output);
        }
        
        public function test_getDataKomentar_gagal(){ //
            //$_SESSION['username'] = 'admin';
            $output = $this->request('GET', 'My_Controller/readDataKomentar');
            //$this->assertRedirect('My_Controller/login');
        }
        
//        public function test_getDataKomentar_gagal(){ // GET DATA KOMENTAR
//            $_SESSION['username'] = "admin";
//            $this->obj->getDataKomentar();
//            $output = $this->request('GET', 'My_Controller/readDataKomentar');
//            //foreach ($data as $row){
//                
//                //$judul 		= $row->judul;
//                $this->assertContains('<title>KOMENTAR</title>', $output);
//            //}
//            //$this->CI->load->view('Komentar', array('komentar' => $data));
//        }
        
        public function test_modelHalamanKomentar() // READ DATA KOMENTAR
        {
            $_SESSION['username'] = "admin";
            $output = $this->request('GET', 'My_Controller/readDataKomentar');                                                      
            $this->assertContains('<title>KOMENTAR</title>', $output);  
...
//            $data = $this->request('POST', 'My_Controller/deletekomentar/141');
